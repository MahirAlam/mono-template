...
> so you might need to disable that in [`@tera/auth`](./packages/auth/src/index.ts)
...
> In this template, we use `@tera` as a placeholder for package names. As a user, you might want to replace it with your own organization or project name. You can use find-and-replace to change all the instances of `@tera` to something like `@my-company` or `@project-name`.
...
3. Outputs a Drizzle-compatible schema file to the `@tera/db` package
